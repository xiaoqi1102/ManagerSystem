...
const RolePanelGroup=React.createClass({
    ...
    handleSubmit(e){
        ...
        if(this.state.isValidateError){
            ...
            Tool.dangerMessage(this.state.message);
            return;
        }
        this.dataPost();
    },
    ...
    dataPost(){
        ...
        YCenterRunTime.ApiCall("更新角色信息接口",JSON.stringify(obj),function(obj){
            console.log(obj);
            that.functionPointPost(obj.data);
        },function(obj){});
    ...
    functionPointPost(id){
        const that=this;
        let checkedPoint=[];
        $.each(that.state.functionPointListData,function(index,functionPointGroup){
       ...
       /* if(this.props.roleId=='null'){
        ...
        YCenterRunTime.ApiCall('更新角色的功能点接口',JSON.stringify(obj),function(obj){
           // that.props.openMessageModal("更新角色功能点成功","success",true);
            Tool.successMessage("更新功能点成功");
            YCenterRunTime.permissionsReload();
            setTimeout(function(){
              //  that.props.toList();
                let host=window.location.host;
                let url=host+'/#/RoleList'
                Tool.openPage(url);
                Tool.messageModalClose(500);
            },1200);
        },function(obj){});
    ...
    getFunctionPointGroup(){
        const that=this;
        if(this.state.functionPointListData==''){
            return(
                <div>
                    <Alert bsStyle="info">
                        功能点暂无数据
            ...
            return(
                <PanelGroup activeKey={this.state.activeKey} onSelect={this.handleSelect} accordion>
                    {this.state.functionPointListData.map(function(functionGroup,index){
                        //   console.log(index);
                        return(
